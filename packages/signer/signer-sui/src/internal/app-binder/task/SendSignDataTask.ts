...
type SignDataTaskArgs = {
  ...
  commandFactory: CommandFactory<Maybe<Signature | SuiAppErrorCodes>>;
};
...
export class SignDataTask {
  ...
  async run(): Promise<
    CommandResult<Maybe<Signature | SuiAppErrorCodes>, SuiAppErrorCodes>
  > {
    ...
    return await new SendCommandInChunksTask<
      Maybe<Signature | SuiAppErrorCodes>
    >(this.api, {
      data: buffer,
      commandFactory,
    }).run();
